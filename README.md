...
这是一个**真实客户项目的脱敏求职展示版本**。原系统已完成核心开发、多轮真实环境测试与客户远程测试，并进入最终交付阶段；本仓库保留原项目的 React 前端、Express 后端、Prisma 数据模型、migration 和回归脚本，移除客户身份、品牌、生产配置与业务数据。
...
以下命令均在仓库根目录执行。本地运行不需要客户服务器或云账号。
...
- 该副本独立于客户原项目，本任务未修改原项目或生产系统；公开仓库仅包含经过脱敏与公开前检查的展示内容。
